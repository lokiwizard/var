...
def build_vae_var(
    # 共享参数
    device, patch_nums=(1, 2, 3, 4, 5, 6, 8, 10, 13, 16),   # 默认 10 个自回归阶段
    # VQVAE 参数
    V=4096, Cvae=32, ch=160, share_quant_resi=4,
    # VAR 参数
    num_classes=1000, depth=16, shared_aln=False, attn_l2_norm=True,
    flash_if_available=True, fused_if_available=True,
    init_adaln=0.5, init_adaln_gamma=1e-5, init_head=0.02, init_std=-1,    # init_std < 0 时自动设置
) -> Tuple[VQVAE, VAR]:
    heads = depth
    width = depth * 64
    dpr = 0.1 * depth/24
    
    # 为了加快构建速度，先关闭 PyTorch 模块默认初始化，后面统一调用 init_weights。
    for clz in (nn.Linear, nn.LayerNorm, nn.BatchNorm2d, nn.SyncBatchNorm, nn.Conv1d, nn.Conv2d, nn.ConvTranspose1d, nn.ConvTranspose2d):
        setattr(clz, 'reset_parameters', lambda self: None)
    
    # 构建冻结的 VQVAE 和待训练的 VAR。
