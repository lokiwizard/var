...
# 本文件只提供 VAR transformer 用到的 3 个基础模块。
__all__ = ['FFN', 'AdaLNSelfAttn', 'AdaLNBeforeHead']


# 自动尝试导入融合算子；环境没有安装时会退回普通 PyTorch 实现。
...
try: from flash_attn import flash_attn_func              # qkv: BLHc，返回 BLHc
except ImportError: pass
...
class SelfAttention(nn.Module):
    def __init__(
        ...
        self.block_idx, self.num_heads, self.head_dim = block_idx, num_heads, embed_dim // num_heads  # 通常为 64
        self.attn_l2_norm = attn_l2_norm
        if self.attn_l2_norm:
            # L2 归一化注意力不再使用固定 1/sqrt(d)，而是学习每个 head 的缩放。
            self.scale = 1
        ...
        # 只在自回归推理时使用：缓存历史 K/V，避免每个尺度重复计算旧 token。
        self.caching, self.cached_k, self.cached_v = False, None, None
    
    def kv_caching(self, enable: bool): self.caching, self.cached_k, self.cached_v = enable, None, None
    
    # 注意：推理时启用 kv cache，attn_bias 会传 None。
    def forward(self, x, attn_bias):
        B, L, C = x.shape
        
        qkv = F.linear(input=x, weight=self.mat_qkv.weight, bias=torch.cat((self.q_bias, self.zero_k_bias, self.v_bias))).view(B, L, 3, self.num_heads, self.head_dim)
        main_type = qkv.dtype
        # qkv: BL3Hc，其中 H 是 head 数，c 是每个 head 的维度。
        
        using_flash = self.using_flash and attn_bias is None and qkv.dtype != torch.float32
        if using_flash or self.using_xform: q, k, v = qkv.unbind(dim=2); dim_cat = 1   # q/k/v: BLHc
        else: q, k, v = qkv.permute(2, 0, 3, 1, 4).unbind(dim=0); dim_cat = 2               # q/k/v: BHLc
        
        if self.attn_l2_norm:
            scale_mul = self.scale_mul_1H11.clamp_max(self.max_scale_mul).exp()
            if using_flash or self.using_xform: scale_mul = scale_mul.transpose(1, 2)  # 1H11 -> 11H1
            q = F.normalize(q, dim=-1).mul(scale_mul)
            k = F.normalize(k, dim=-1)
        
        if self.caching:
            # 自回归推理时，每次只追加当前尺度的 K/V；历史 K/V 直接复用。
...
class AdaLNSelfAttn(nn.Module):
    ...
    # 注意：推理时启用 kv cache，attn_bias 会传 None。
    def forward(self, x, cond_BD, attn_bias):   # C 为 embed_dim，D 为 cond_dim
        if self.shared_aln:
            gamma1, gamma2, scale1, scale2, shift1, shift2 = (self.ada_gss + cond_BD).unbind(2) # 116C + B16C，经 unbind(2) 得到 6 个 B1C
        else:
            gamma1, gamma2, scale1, scale2, shift1, shift2 = self.ada_lin(cond_BD).view(-1, 1, 6, self.C).unbind(2)
        # AdaLN 的 scale/shift 先调制归一化后的输入，gamma 再控制残差分支强度。
        x = x + self.drop_path(self.attn( self.ln_wo_grad(x).mul(scale1.add(1)).add_(shift1), attn_bias=attn_bias ).mul_(gamma1))
        x = x + self.drop_path(self.ffn( self.ln_wo_grad(x).mul(scale2.add(1)).add_(shift2) ).mul(gamma2)) # 使用 FusedMLP 时这里的 mul(gamma2) 不能原地执行
        return x
...
class AdaLNBeforeHead(nn.Module):
    def __init__(self, C, D, norm_layer):   # C 为 embed_dim，D 为 cond_dim
        super().__init__()
        self.C, self.D = C, D
