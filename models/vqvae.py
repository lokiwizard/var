...
class VQVAE(nn.Module):
    def __init__(
        ...
        beta=0.25,              # commitment 损失权重
        using_znorm=False,      # 查找最近 codebook 向量时是否做归一化
        quant_conv_ks=3,        # quant_conv 的卷积核大小
        quant_resi=0.5,         # 0.5 表示 \phi(x) = 0.5conv(x) + (1-0.5)x
        share_quant_resi=4,     # K 个尺度共享 4 个 \phi 层，即部分共享 \phi
        default_qresi_counts=0, # 为 0 时自动设置为 len(v_patch_nums)
        v_patch_nums=(1, 2, 3, 4, 5, 6, 8, 10, 13, 16), # 每个尺度的 patch 数，h_{1 to K}=w_{1 to K}=v_patch_nums[k]
        test_mode=True,
    ):
        super().__init__()
        self.test_mode = test_mode
        self.V, self.Cvae = vocab_size, z_channels
        # ddconfig 参考 latent-diffusion 的 vq-f16 配置。
        ddconfig = dict(
            dropout=dropout, ch=ch, z_channels=z_channels,
            in_channels=3, ch_mult=(1, 1, 2, 2, 4), num_res_blocks=2,   # 来自 vq-f16/config.yaml
            using_sa=True, using_mid_sa=True,                           # 来自 vq-f16/config.yaml
            # resamp_with_conv=True,   # 原实现中始终为 True，这里删去显式配置。
        )
        ddconfig.pop('double_z', None)  # 只有 KL-VAE 才应使用 double_z=True
    ...
    # ===================== `forward` 仅用于训练 VAE =====================
    def forward(self, inp, ret_usages=False):   # -> rec_B3HW, idx_N, 损失
    ...
    def img_to_idxBl(self, inp_img_no_grad: torch.Tensor, v_patch_nums: Optional[Sequence[Union[int, Tuple[int, int]]]] = None) -> List[torch.LongTensor]:    # 返回 List[Bl]
