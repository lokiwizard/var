...
# 本文件只提供 VQVAE 使用的 Encoder 和 Decoder。
__all__ = ['Encoder', 'Decoder',]


"""
参考实现: https://github.com/CompVis/stable-diffusion/blob/21f890f9da3cfbeaba8e2ac3c425ee9e998d5229/ldm/modules/diffusionmodules/model.py
"""
# swish 激活函数
def nonlinearity(x):
...
class ResnetBlock(nn.Module):
    def __init__(self, *, in_channels, out_channels=None, dropout): # conv_shortcut=False，VAE 中始终不使用 conv_shortcut
        super().__init__()
...
class AttnBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        w = w.permute(0, 2, 1).contiguous()  # B,HW,HW（第一个 HW 对应 k，第二个 HW 对应 q）
        h = torch.bmm(v, w)  # B,C,HW（HW 对应 q），h[B,C,j] = sum_i v[B,C,i] w[B,i,j]
...
class Decoder(nn.Module):
    def __init__(
        self, *, ch=128, ch_mult=(1, 2, 4, 8), num_res_blocks=2,
        dropout=0.0, in_channels=3,  # in_channels 表示原始图像通道数
        ...
        for i_level in reversed(range(self.num_resolutions)):
            ...
            self.up.insert(0, up)  # 前插，保持索引顺序与分辨率层级一致
