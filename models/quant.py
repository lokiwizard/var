...
# 本文件提供 VQVAE 使用的残差多尺度向量量化器。
# 它把连续潜变量图转成一组离散码本 id 金字塔。
__all__ = ['VectorQuantizer2',]


class VectorQuantizer2(nn.Module):
    # VQGAN 原版使用 beta=1.0；Stable Diffusion 系列通常使用 0.25。
    def __init__(
        ...
        # phi 是作用在每个量化尺度上的小残差卷积。
        # 码本向量上采样后先经过 phi，再累加到当前潜变量重建中。
        if share_quant_resi == 0:   # 不共享：K 个尺度各自使用 \phi_1 到 \phi_K
        ...
        # 仅用于 VAR 的渐进式训练；当前默认不启用，prog_si 通常保持 -1。
        self.prog_si = -1
    ...
    def forward(self, f_BChw: torch.Tensor, ret_usages=False) -> Tuple[torch.Tensor, List[float], torch.Tensor]:
        ...
        with torch.cuda.amp.autocast(enabled=False):
            ...
            for si, pn in enumerate(self.v_patch_nums): # 从小尺度到大尺度
                # 在当前尺度量化剩余残差。粗尺度使用 area pooling 后的残差；
                # 最后一个尺度直接使用原始潜变量网格。
                if self.using_znorm:
                ...
                # 将选中的码本向量还原到最终潜变量尺寸，经过 phi 后加入 f_hat；
                # 同时从 f_rest 中扣除，让更细尺度只负责解释前面没解释掉的残差。
    ...
    def embed_to_fhat(self, ms_h_BChw: List[torch.Tensor], all_to_max_scale=True, last_one=False) -> Union[List[torch.Tensor], torch.Tensor]:
        ...
        if all_to_max_scale:
            ...
            # 注意：这不是 VQ-VAE 训练或推理的常规路径；常规路径会像上面一样
            # 把每个 token map 插值到最大 H/W。这里仅用于实验。
    ...
    def f_to_idxBl_or_fhat(self, f_BChw: torch.Tensor, to_fhat: bool, v_patch_nums: Optional[Sequence[Union[int, Tuple[int, int]]]] = None) -> List[Union[torch.Tensor, torch.LongTensor]]:  # f_BChw 是输入图像编码后的潜变量特征
        B, C, H, W = f_BChw.shape
        f_no_grad = f_BChw.detach()
        f_rest = f_no_grad.clone()
        f_hat = torch.zeros_like(f_rest)
        ...
        for si, (ph, pw) in enumerate(patch_hws): # 从小尺度到大尺度
            if 0 <= self.prog_si < si: break    # 渐进式训练时只取到当前阶段；默认 prog_si=-1 不截断
            # 与 forward() 相同的残差量化循环，但这里返回两种结果：
            # 扁平 token id（作为 VAR 训练目标）或累计 f_hat map（用于重建/调试可视化）。
    ...
    # ===================== idxBl_to_var_input 仅用于 VAR 训练，用来构造教师强制输入 =====================
    def idxBl_to_var_input(self, gt_ms_idx_Bl: List[torch.Tensor]) -> torch.Tensor:
        # 第 s 个尺度的教师强制输入不是第 s 个尺度的真值 id，
        # 而是由所有更早尺度重建出的累计潜变量，再下采样到当前 pn x pn 网格。
        next_scales = []
        ...
        for si in range(SN-1):
            if self.prog_si == 0 or (0 <= self.prog_si-1 < si): break   # 渐进式训练时截断教师强制输入
        ...
        return torch.cat(next_scales, dim=1) if len(next_scales) else None    # 将多个 BlC 拼成 BLC，保持 float32
    
    # ===================== get_next_autoregressive_input 仅用于 VAR 推理，用来构造下一步输入 =====================
    def get_next_autoregressive_input(self, si: int, SN: int, f_hat: torch.Tensor, h_BChw: torch.Tensor) -> Tuple[Optional[torch.Tensor], torch.Tensor]: # 仅用于 VAR 推理
        HW = self.v_patch_nums[-1]
        if si != SN-1:
            # 采样时把当前尺度新增的残差加到 f_hat，
            # 并把累计潜变量的低分辨率视图提供给下一尺度。
            h = self.quant_resi[si/(SN-1)](F.interpolate(h_BChw, size=(HW, HW), mode='bicubic'))     # 先上采样再过 conv
            f_hat.add_(h)
...
class Phi(nn.Conv2d):
    ...
    def forward(self, h_BChw):
        # 混合 identity 和 convolution：quant_resi=0 时接近恒等映射，
        # quant_resi 越大，phi 越能修正插值/codebook 带来的偏差。
