...
def build_everything(args: arg_util.Args):
    ...
    # 创建 tensorboard logger；只在 master rank 上真正写日志。
    ...
    # 构建模型。VQVAE 用预训练权重并冻结，VAR 是训练目标。
    ...
    vae_local, var_wo_ddp = build_vae_var(
        V=4096, Cvae=32, ch=160, share_quant_resi=4,        # 固定的 VQVAE 超参数
    ...
    # 构建优化器；filter_params 会把不做权重衰减的参数单独分组。
    ...
    # 构建 trainer，并在需要时恢复 VAR/optimizer 状态；VAE 权重已单独加载。
    trainer = VARTrainer(
    ...
    if trainer_state is not None and len(trainer_state):
        trainer.load_state_dict(trainer_state, strict=False, skip_vae=True) # 不再重复加载 VAE
...
def train_one_ep(ep: int, is_first_ep: bool, start_it: int, args: arg_util.Args, tb_lg: misc.TensorboardLogger, ld_or_itrt, iters_train: int, trainer):
    # 在 DataLoader 创建后再导入较重模块，减少多进程初始化成本。
    from trainer import VARTrainer
    ...
    for it, (inp, label) in me.log_every(start_it, iters_train, ld_or_itrt, 30 if iters_train > 8000 else 5, header):
        ...
        if args.pg: # 默认 args.pg == 0.0，表示不启用渐进式训练，不会进入该分支。
            if g_it <= wp_it: prog_si = args.pg0
            elif g_it >= max_it*args.pg: prog_si = len(args.patch_nums) - 1
            else:
                delta = len(args.patch_nums) - 1 - args.pg0
                progress = min(max((g_it - wp_it) / (max_it*args.pg - wp_it), 0), 1) # 从 0 递增到 1
                prog_si = args.pg0 + round(progress * delta)    # 从 args.pg0 递增到 len(args.patch_nums)-1
        else:
            prog_si = -1
    ...
    return {k: meter.global_avg for k, meter in me.meters.items()}, me.iter_time.time_preds(max_it - (g_it + 1) + (args.ep - ep) * 15)  # +15 预留验证/保存等额外耗时
