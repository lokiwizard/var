...
class AmpOptimizer:
    def __init__(
        ...
        if self.enable_amp:
            ...
            self.scaler = torch.cuda.amp.GradScaler(init_scale=2. ** 11, growth_interval=1000) if self.using_fp16_rather_bf16 else None # 只有 fp16 需要 scaler
        else:
            self.amp_ctx = NullCtx()
            self.scaler = None
        
        self.optimizer, self.names, self.paras = optimizer, names, paras   # paras 已经过滤，全部 requires_grad=True
        ...
        self.r_accu = 1 / n_gradient_accumulation   # r_accu 等于 1.0 / n_gradient_accumulation
    
    def backward_clip_step(
        self, stepping: bool, loss: torch.Tensor,
    ) -> Tuple[Optional[Union[torch.Tensor, float]], Optional[float]]:
        # 反向传播；梯度累积时先把损失按累积步数缩放。
        loss = loss.mul(self.r_accu)   # r_accu 等于 1.0 / n_gradient_accumulation
        ...
        if stepping:
            ...
            if self.scaler is not None:
                ...
                if scaler_sc > 32768.: # fp16 超过 65536 会溢出，因此继续放大到 32768 以上有风险。
